...
# Evidence-only candidate matrix.  This file deliberately mutates no Product code.
# 264 independently discoverable unittest methods exercise DEV2-owned canonical
# state/history/FEN/atomicity contracts on the exact Stage1 repair candidate.
...
# 001-064: every canonical square index round-trips through the shared square core.
...
for _index in range(64):
    setattr(
        Dev2Stage1Candidate264Matrix,
        f"test_001_064_square_roundtrip_{_index:02d}",
        _make_square_roundtrip_case(_index),
    )


# 065-128: all 64 text squares normalize uppercase/padded input without identity drift.
...
for _index in range(64):
    setattr(
        Dev2Stage1Candidate264Matrix,
        f"test_065_128_square_normalization_{_index:02d}",
        _make_square_normalization_case(_index),
    )


# 129-160: white/black move-number review targets map to exact stable plies.
...
for _move_number in range(1, 17):
    for _side in ("w", "b"):
        setattr(
            Dev2Stage1Candidate264Matrix,
            f"test_{_case:03d}_history_jump_{_move_number:02d}{_side}",
            _make_history_jump_case(_move_number, _side),
        )
        _case += 1


# 161-184: selecting each stable node preserves node identity, FEN and lineage ply.
...
for _node_id in range(1, 25):
    setattr(
        Dev2Stage1Candidate264Matrix,
        f"test_{160 + _node_id:03d}_history_select_node_{_node_id:02d}",
        _make_history_node_case(_node_id),
    )


# 185-200: exact FEN counter/turn round-trips, including non-zero halfmove/fullmove.
...
for _index in range(16):
    setattr(
        Dev2Stage1Candidate264Matrix,
        f"test_{185 + _index:03d}_fen_counter_roundtrip_{_index:02d}",
        _make_fen_counter_case(_index),
    )
...
_INVALID_FENS = (
    ...
    "7k/8/8/8/8/8/8/K7 w - - 0 -1",
    "7k/8/8/8/8/8/8/K7 w - - 1.0 1",
    "7k/8/8/8/8/8/8/K7 w - - 0 1.0",
    "7k/8/8/8/8/8/8/K7 w - - 1e3 1",
    "7k/8/8/8/8/8/8/K7 w - - 0 1e3",
    "7k/8/8/8/8/8/8/K7 w - - ٠ 1",
    "7k/8/8/8/8/8/8/K7 w - - 0 ١",
    "7k/8/8/8/8/8/8/K7 w - - NaN 1",
    "7k/8/8/8/8/8/8/K7 w - - 0 NaN",
    "7k/8/8/8/8/8/8/K7 w - - 0x10 1",
    "7k/8/8/8/8/8/8/K7 w - - 0 0x10",
    "7k/8/8/8/8/8/8/K7 w - - 0 0",
    "7k/8/8/8/8/8/8/K7 w - - 0 1 extra",
)


# 201-216: malformed FEN attempts are atomic for canonical board/history/redo/last-move state.
...
for _index in range(16):
    setattr(
        Dev2Stage1Candidate264Matrix,
        f"test_{201 + _index:03d}_invalid_fen_atomicity_{_index:02d}",
        _make_invalid_fen_atomicity_case(_index),
    )


# 217-232: repeated undo/redo cycles restore the exact canonical FEN and stack topology.
...
for _cycles in range(1, 17):
    setattr(
        Dev2Stage1Candidate264Matrix,
        f"test_{216 + _cycles:03d}_undo_redo_cycles_{_cycles:02d}",
        _make_undo_redo_case(_cycles),
    )


# 233-248: non-activating branch insertion is idempotent and preserves old futures/cursor.
...
for _index in range(16):
    setattr(
        Dev2Stage1Candidate264Matrix,
        f"test_{233 + _index:03d}_history_branch_preservation_{_index:02d}",
        _make_branch_case(_index),
    )


# 249-256: all files accept a structurally real prior black double-pawn push.
...
for _file_index in range(8):
    setattr(
        Dev2Stage1Candidate264Matrix,
        f"test_{249 + _file_index:03d}_valid_en_passant_provenance_{_file_index}",
        _make_valid_ep_case(_file_index),
    )


# 257-264: the alleged pawn origin remaining occupied invalidates en-passant provenance atomically.
...
for _file_index in range(8):
    setattr(
        Dev2Stage1Candidate264Matrix,
        f"test_{257 + _file_index:03d}_invalid_en_passant_origin_{_file_index}",
        _make_invalid_ep_origin_case(_file_index),
    )
