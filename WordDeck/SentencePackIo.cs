...
internal static class SentencePackIo
{
    ...
    internal static bool HasGZipSignature(string path)
    {
    ...
    public static SentencePack Read(string path)
    {
        ...
        bool gzip = HasGZipSignature(fullPath);
        using FileStream file = new(fullPath, FileMode.Open, FileAccess.Read, FileShare.Read);
        if (gzip)
        {
            using var decompressor = new GZipStream(file, CompressionMode.Decompress, leaveOpen: false);
            using var limited = new LimitedReadStream(decompressor, MaxDecompressedJsonBytes);
            return ReadCore(limited);
        }

        // Content, not a temporary/staging suffix, decides whether the file is gzip. This lets
        // transaction staging use .tmp files without accidentally parsing compressed bytes as JSON.
