...
class ChessBaseBundleManifest:
    ...
    def as_dict(self) -> dict[str, object]:
        def item(e: ComponentEvidence) -> dict[str, object]:
            return {"path": e.path, "extension": e.extension, "role": e.role, "size": e.size, "sha256": e.sha256}
        return {
...
def _hash_file(path: Path, chunk_size: int = 1024 * 1024) -> ComponentEvidence:
    ...
    with path.open("rb") as handle:
        while True:
            ...
            size += len(chunk)
            digest.update(chunk)
    return ComponentEvidence(str(path.resolve()), path.suffix.lower(), "source", size, digest.hexdigest())
...
def build_chessbase_manifest(path: str | Path) -> ChessBaseBundleManifest:
    ...
    if not probe.recognized:
        ...
        return ChessBaseBundleManifest(MANIFEST_SCHEMA_VERSION, str(source.resolve()), probe.source_kind, probe.family_name, "unsupported", None, warnings=tuple(warnings))
    if not probe.is_primary_source:
        warnings.append("Component-only path is not a database primary source.")
        return ChessBaseBundleManifest(MANIFEST_SCHEMA_VERSION, str(source.resolve()), probe.source_kind, probe.family_name, "component_only", None, warnings=tuple(warnings))
    if not source.is_file():
        warnings.append("Primary source file is missing or unavailable.")
        return ChessBaseBundleManifest(MANIFEST_SCHEMA_VERSION, str(source.resolve()), probe.source_kind, probe.family_name, "damaged", None, warnings=tuple(warnings))
    ...
    if probe.extension == ".cbh":
        for component in probe.components:
            if component.exists and component.path.is_file():
                components.append(_with_role(_hash_file(component.path), component.role, component.extension))
        if not components:
    ...
    return ChessBaseBundleManifest(MANIFEST_SCHEMA_VERSION, str(source.resolve()), probe.source_kind, probe.family_name, status, primary, tuple(components), tuple(warnings))
...
def verify_manifest_unchanged(manifest: ChessBaseBundleManifest) -> tuple[bool, tuple[str, ...]]:
    ...
    for evidence in manifest.all_evidence:
        ...
        if current.size != evidence.size:
            problems.append(f"Size changed for {evidence.path}: {evidence.size} -> {current.size}")
