...
class PgnDocumentSession:
    ...
    def open(cls, path: str | Path) -> "PgnDocumentSession":
        ...
        workspace = PgnWorkspace(opened.games)
        overwrite_safe = not any(
            "Invalid UTF-8 bytes were replaced" in warning
            for warning in opened.global_warnings
        )
        return cls(
            workspace,
            source=opened.source,
            global_warnings=opened.global_warnings,
    ...
    def append_text(self, text: object) -> int:
        ...
        incoming = imported.games()
        combined = self._workspace.games() + incoming
    ...
    def save(self) -> SourceFingerprint:
        ...
        if not self._source_overwrite_safe:
            raise _error(
                "source decoding replaced invalid bytes; use Save As to preserve the original",
