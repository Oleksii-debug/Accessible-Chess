...
// Platform-neutral contracts for the learning engine. WinForms remains a presentation
// adapter; future Blazor/server integrations can consume these DTOs without depending
// on controls, focus state, screen coordinates or a network connection.
    ...
    ImageRevealPolicy RevealPolicy)
{
    public void ValidateForEntry(string englishAnswer)
    {
        ...
        {
            if (ContainsWholeAnswer(AltText, answer) || ContainsWholeAnswer(HintText, answer))
...
internal sealed class OfflineWordImageProvider : IWordImageProvider
{
    ...
    {
        if (string.IsNullOrWhiteSpace(dictionaryId)) throw new ArgumentException("Dictionary id is required.", nameof(dictionaryId));
        if (string.IsNullOrWhiteSpace(entryId)) throw new ArgumentException("Entry id is required.", nameof(entryId));
