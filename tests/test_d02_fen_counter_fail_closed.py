...
class D02FenCounterFailClosedTests(unittest.TestCase):
    @staticmethod
    ...
    def test_board_oversized_halfmove_is_normalized_to_existing_domain_error(self) -> None:
        with _bounded_int_string_limit():
            with self.assertRaises(ValueError) as raised:
                Board(self._fen(halfmove="9" * 700))
        self.assert_domain_error(str(raised.exception))

    def test_board_oversized_fullmove_is_normalized_to_existing_domain_error(self) -> None:
        with _bounded_int_string_limit():
            with self.assertRaises(ValueError) as raised:
                Board(self._fen(fullmove="9" * 700))
        self.assert_domain_error(str(raised.exception))
    ...
    def test_api_oversized_halfmove_is_concise_and_atomic(self) -> None:
        self._assert_api_failure_is_concise_and_atomic(
            self._fen(halfmove="9" * 700)
        )

    def test_api_oversized_fullmove_is_concise_and_atomic(self) -> None:
        self._assert_api_failure_is_concise_and_atomic(
            self._fen(fullmove="9" * 700)
        )

    def test_large_representable_counters_are_not_arbitrarily_capped(self) -> None:
        halfmove = "8" * 639
        fullmove = "7" * 639
        fen = self._fen(halfmove=halfmove, fullmove=fullmove)
        with _bounded_int_string_limit():
            board = Board(fen)
            self.assertEqual(board.fen(), fen)
        self.assertEqual(board.halfmove, int(halfmove))
        self.assertEqual(board.fullmove, int(fullmove))
