    ...
    public static void ValidateForInstallation(SentencePack pack)
    {
        ...
        foreach (SentenceRecord sentence in pack.Sentences)
        {
            if (!string.Equals(sentence.License, pack.License, StringComparison.Ordinal))
                throw new InvalidDataException($"SentencePack {pack.PackId} mixes sentence license '{sentence.License}' with pack license '{pack.License}'. Mixed-license packs are not accepted by this release format.");
        }
        ...
        if (!string.Equals(pack.License, TatoebaCcBy, StringComparison.Ordinal)) return;
