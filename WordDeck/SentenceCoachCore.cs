...
internal sealed class SentencePack : ISentenceCorpus
{
    ...
    public IReadOnlyList<SentenceRecord> LookupByLemma(string lemma)
    {
        ...
        return normalized.Length > 0 && _byLemma!.TryGetValue(normalized, out List<SentenceRecord>? values)
            ? values
            : Array.Empty<SentenceRecord>();
    }
...
internal static partial class SentenceTokenizer
{
    ...
    {
        string normalizedText = NormalizeCompatibilityText(text);
    ...
    public static string NormalizeToken(string token) =>
        NormalizeCompatibilityText(token).Trim().ToLowerInvariant();
}
...
internal static class SentenceAnswerEvaluator
{
    ...
    private static string BuildFeedback(
        IReadOnlyList<string> missing,
        IReadOnlyList<string> extra,
        IReadOnlyList<string> misspellings)
    {
...
internal sealed class SentenceSelector
{
    ...
    public SentenceSelectionResult? Select(IReadOnlyList<string> targetEntryIds, SentenceSelectionContext context)
    {
        ...
        IReadOnlyList<SentenceRecord> candidates = _corpus.LookupAllTargets(targets);
        if (candidates.Count > 0)
        {
            SentenceRecord best = candidates
                .OrderBy(sentence => Score(sentence, targets, context))
                .ThenBy(sentence => sentence.Id, StringComparer.Ordinal)
                .First();
            return new SentenceSelectionResult(
                best,
                false,
                "Selected an indexed corpus sentence containing every requested target and ranked it by personal/CEFR context difficulty.");
        }

        SentenceRecord? generated = _fallback?.TryGenerate(targets, context);
        if (generated is null) return null;
        generated.Validate();
        if (!targets.All(target => generated.TargetEntryIds.Contains(target, StringComparer.OrdinalIgnoreCase)))
    ...
    internal static int Score(SentenceRecord sentence, IReadOnlyList<string> targets, SentenceSelectionContext context)
    {
        ...
        int aboveLevelUnknown = sentence.EntryLevels.Count(pair =>
            !targetSet.Contains(pair.Key) &&
            !context.KnownEntryIds.Contains(pair.Key) &&
            LevelValue(pair.Value) > targetLevel);
    ...
    private static int LevelValue(string level) => level.ToUpperInvariant() switch
    {
        ...
        "B2" => 4,
        "C1" => 5,
        _ => throw new InvalidDataException($"Unsupported Sentence Coach CEFR level '{level}'.")
    };
}
...
internal static class SentencePackJson
{
    public static SentencePack Parse(string json)
    {
        try
        {
            SentencePack pack = JsonSerializer.Deserialize<SentencePack>(
                json,
        ...
        {
            throw new InvalidDataException("SentencePack JSON is malformed.", ex);
        }
    }
