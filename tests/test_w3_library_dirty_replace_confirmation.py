...
class W3LibraryDirtyReplaceConfirmationEvidenceTests(unittest.TestCase):
    ...
    def test_production_composition_binds_exact_owner_confirmation_after_close_guard(self) -> None:
        ...
        close_guard = source.index(
            "file_runtime = _install_close_guard_or_shutdown(", runtime_build
        )
        binding = source.index("application.confirm_document_replace = (", close_guard)
        owner_confirmation = source.index(
            "file_runtime.file_dialogs.confirm_discard_unsaved_pgn", binding
        )
        runtime_return = source.index("return file_runtime", owner_confirmation)

        self.assertLess(runtime_build, close_guard)
        self.assertLess(close_guard, binding)
        self.assertLess(binding, owner_confirmation)
        self.assertLess(owner_confirmation, runtime_return)
        self.assertIn(
