...
def locate_cbg_move_payload_from_header(
    header: ClassicCbgGameHeader,
) -> ClassicCbgMovePayloadSpan:
    """Derive an opaque payload span from one already validated CBG header."""

    if not isinstance(header, ClassicCbgGameHeader):
        raise TypeError("header must be a ClassicCbgGameHeader")
    if header.unsupported_reasons:
        reasons = ", ".join(header.unsupported_reasons)
        raise CbgDecodeError(
            f"CBG move payload at offset {header.offset} is unsupported: {reasons}"
        )
    ...
    if header.game_length < relative_payload_start:
        raise CbgDecodeError(
            f"CBG move payload at offset {header.offset} is truncated: "
            f"declared game length {header.game_length}, "
            f"need at least {relative_payload_start} bytes"
        )

    game_end = header.offset + header.game_length
    payload_start = header.offset + relative_payload_start
    return ClassicCbgMovePayloadSpan(
        game_offset=header.offset,
...
def locate_cbg_move_payload(data: bytes, *, offset: int) -> ClassicCbgMovePayloadSpan:
    ...
    return locate_cbg_move_payload_from_header(
        parse_cbg_game_header(data, offset=offset)
    )
