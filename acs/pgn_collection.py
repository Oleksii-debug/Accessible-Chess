...
from .game_references import PositionRef, VariationRef
    ...
    *,
    ...
    for incoming_index, game in enumerate(incoming):
        ...
        if usable_only and not semantic.usable:
            decisions.append(MergeDecision(incoming_index, fingerprint, "skipped_unusable", None, duplicate_of, semantic.usable, semantic.warning_count, semantic.error_count))
            continue

        if duplicate_policy != DuplicatePolicy.KEEP and duplicate_of is not None:
            decisions.append(MergeDecision(incoming_index, fingerprint, "skipped_duplicate", None, duplicate_of, semantic.usable, semantic.warning_count, semantic.error_count))
            continue
        ...
        decisions.append(MergeDecision(incoming_index, fingerprint, "added", assigned, duplicate_of, semantic.usable, semantic.warning_count, semantic.error_count))

    merged = PgnWorkspace((*existing, *accepted))
    return merged, MergeReport(tuple(decisions), len(existing), len(merged))


def search_page(workspace: PgnWorkspace, query: PgnQuery, *, offset: int = 0, limit: int = 100) -> SearchPage:
    if offset < 0:
...
def export_pages(workspace: PgnWorkspace, *, games_per_page: int = 1000) -> tuple[str, ...]:
    ...
    return tuple(serialize_games(games[index : index + games_per_page]) for index in range(0, len(games), games_per_page))
