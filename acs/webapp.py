...
class AccessibleChessAPI:
    ...
    def _pieces_text(self, color: str) -> str:
        ...
        lines: list[str] = []
        for typ in "KQRBNP":
            squares = [_spaced_square(sq_name(i)) for i, p in enumerate(self.board.board) if p and p.upper() == typ and color_of(p) == color]
            if squares:
    ...
    def _moves_text(self) -> str:
        ...
        while i < len(self.sans):
            side = self.move_sides[i]
            if side == "w":
                white = _spoken_san(self.sans[i], self.lang)
                if i + 1 < len(self.sans) and self.move_sides[i + 1] == "b":
                    black = _spoken_san(self.sans[i + 1], self.lang)
                ...
                out.append(f"{move_no}... {_spoken_san(self.sans[i], self.lang)}.")
                move_no += 1
                i += 1
        return "\n".join(out)
    ...
    def _ok(self, message: str) -> dict[str, Any]:
        ...
        state = self.get_state(); state["ok"] = True
        return state

    def _error(self, message: str) -> dict[str, Any]:
        self.announcement = message
        state = self.get_state(); state["ok"] = False
        return state
    ...
    def make_move(self, text: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.sans.append(san); self.move_sides.append(side); self.redo_meta.clear(); self.selected_source = None
    ...
    def undo(self) -> dict[str, Any]:
        ...
        if san is None:
            return self._error(self._t("undo_none"))
        side = self.move_sides.pop(); self.sans.pop(); self.redo_meta.append((san, side)); self.selected_source = None
    ...
    def redo(self) -> dict[str, Any]:
        ...
        san = self.board.redo()
        if san is None:
            self.redo_meta.clear(); return self._error(self._t("redo_none"))
        meta_san, side = self.redo_meta.pop(); self.sans.append(meta_san); self.move_sides.append(side); self.selected_source = None
    ...
    def set_turn(self, color: str) -> dict[str, Any]:
        ...
        self.board.turn = color; self.selected_source = None
    ...
    def diagnostic(self) -> dict[str, Any]:
        ...
        return {"ok": True, "version": VERSION, "boardCells": len(self._board_cells()),
                ...
                "semanticDocumentPresent": semantic}


def _make_menu(webview: Any, api: AccessibleChessAPI, window_holder: dict[str, Any]):
    Menu = webview.menu.Menu; MenuAction = webview.menu.MenuAction; MenuSeparator = webview.menu.MenuSeparator
    def js(code: str) -> None:
        w = window_holder.get("window")
        if w:
            try: w.evaluate_js(code)
...
def main() -> None:
    import webview
    api = AccessibleChessAPI(); window_holder: dict[str, Any] = {}
    html = _asset_root() / "web" / "index.html"
    if not html.exists(): raise RuntimeError(f"Accessible HTML UI not found: {html}")
    menu = _make_menu(webview, api, window_holder)
    window = webview.create_window("Accessible Chess — 0.4 NVDA architecture", url=str(html), js_api=api,
                                   width=1150, height=820, min_size=(800, 600), text_select=True, menu=menu)
