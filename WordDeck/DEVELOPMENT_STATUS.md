...
- The post-`mutual` through `offspring` QA ledger now accounts for **57 B2/C1 lexical candidates**. **28 rows are `verified` but deliberately not runtime-activated** after Oxford definition/sense review and Ukrainian translation QA. They are isolated in `QA/oxford5000_source_after_mutual_verified_c1_0001_0028.tsv`. **29 rows remain `ambiguous_source`** in `QA/oxford5000_source_after_mutual_ambiguous_b2c1_0001_0029.tsv`: their POS/CEFR is confirmed in the current official Oxford union, but exact Oxford5000-exclusive membership still needs official membership-preserving reconciliation. All 29 remain blank/fail-closed. The obsolete mixed staging file was deleted to prevent duplicate-ledger/activation risk.
- Verified-not-activated examples include `namely`, `nationwide`, `naval`, `niche`, `noble`, `nominate`, `non-profit`, `nonetheless`, `notorious`, `novel` adjective, `nursery`, `oblige`, `obsess`, `occurrence`, `odds`, `offering` and `offspring`. Polysemous translations preserve multiple relevant Ukrainian senses rather than flattening them.
...
- **Exact next data action:** obtain/use a saved official Oxford word-list source that preserves Oxford3000-vs-Oxford5000 membership, run full reconciliation to turn global `remaining` from UNKNOWN into an exact count, resolve the 29 source-ambiguous rows, and continue the global corpus independently of ambiguous entries. The isolated 28-row verified slice is technically ready for a controlled runtime integration only after a visible successful Windows gate is available; do not promote it merely because translation QA is complete.
...
- Post-`mutual` QA now contains **57 B2/C1 lexical candidates**: 28 verified-not-activated in a clean C1 slice and 29 source-ambiguous in a separate fail-closed B2/C1 file. None is runtime-active. This replaces the incorrect earlier claim that only 29 C1 rows represented that interval.
