...
This module owns only the filesystem boundary between an already prepared
product/notices pair and the canonical Version 2 package tree/ZIP. It does not
compile the Windows executable, download engines, invent release authority, or
publish a real candidate. Every assembled tree and archive is accepted only
through the one canonical :mod:`acs.version2_package_preflight` validator.
"""
        ...
        for dirpath, dirnames, filenames in walker:
            ...
            for name in filenames:
                ...
                try:
                    ...
                    # Source metadata is non-authoritative. Exact bytes and the
                    # later canonical checksum/preflight are authoritative.
                    pass
...
def _publish_directory_no_replace(staged: Path, output: Path) -> None:
    """Atomically rename a validated directory without ever replacing output."""
    ...
    if os.name == "nt":
        ...
        if move_file_ex(str(staged), str(output), 0):
            return
        error = ctypes.get_last_error()
        if error in _WINDOWS_ALREADY_EXISTS or _path_entry_exists(
            output, label="package output"
        ):
            _fail("package output appeared during assembly and will not be overwritten")
        _fail(f"validated package could not be published atomically: WinError {error}")
    ...
    if sys.platform.startswith("linux"):
        ...
        if error in {errno.EEXIST, errno.ENOTEMPTY}:
            _fail("package output appeared during assembly and will not be overwritten")
        _fail(
            "validated package could not be published atomically: "
...
def assemble_version2_package_tree(
    ...
    *,
    ...
    if _path_entry_exists(output, label="package output"):
    ...
    try:
        ...
        report = validate_version2_package_tree(staged, expected_integration_sha=sha)
    ...
    *,
    ...
    try:
        ...
        with zipfile.ZipFile(
            ...
            for relative in sorted(tree_report.inventory, key=str.casefold):
