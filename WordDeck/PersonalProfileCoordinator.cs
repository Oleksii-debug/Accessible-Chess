...
internal sealed class PersonalProfileCoordinator
{
    ...
    public void Export(AppState state, string destinationPath)
    {
        ...
        try
        {
            ...
            // Read back before publishing the file so a truncated/invalid export never replaces a good destination.
        ...
        IEnumerable<string> knownDictionaryIds)
    {
        ...
        if (!string.Equals(envelope.CorpusIdentity, AppStateStore.CorpusIdentity, StringComparison.Ordinal))
        {
            throw new InvalidDataException(
                $"Profile corpus identity '{envelope.CorpusIdentity}' is incompatible with this WordDeck build. No personal state was changed.");
        }

        SentenceCoachState? importedSentence = envelope.SentenceState is null
            ? null
            : ValidateSentenceState(envelope.SentenceState, knownEntryIds);
        ...
        if (importedSentence is not null && !string.IsNullOrWhiteSpace(importedSentence.ActivePackId))
        {
            ...
            {
                throw new InvalidDataException(
                    $"Profile references SentencePack '{importedSentence.ActivePackId}', which is not installed and validated on this device. No personal state was changed.");
            }
        }
        ...
        try
        {
            if (importedSentence is not null)
            {
                _sentenceStore.Save(importedSentence);
                sentenceChanged = true;
            }

            return _appStore.ImportProfile(fullSource, destination, knownEntryIds, knownDictionaryIds);
        }
        catch
        {
            if (sentenceChanged)
    ...
    private static SentenceCoachState ValidateSentenceState(SentenceCoachState source, IEnumerable<string> knownEntryIds)
    {
        ...
        var known = new HashSet<string>(knownEntryIds.Where(id => !string.IsNullOrWhiteSpace(id)), StringComparer.OrdinalIgnoreCase);
