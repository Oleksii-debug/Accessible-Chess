...
class Stage1ReleaseAccessibleChessAPI(KeymapAwareAccessibleChessAPI):
    ...
    def activate_square(self, square: str) -> dict[str, Any]:
        ...
        # Preserve deliberate chess/application messages.  Replace only an
        # unexpected lower-level failure with one concise action error.
        allowed = {
        ...
        if message in allowed or message.startswith("Зараз хід іншої сторони") or message.startswith("It is the other side's turn"):
            return result
        if message and not any(token in message for token in ("Traceback", "ValueError", "RuntimeError", "Exception", " at 0x")):
            return result
        return self._concise_error("Не вдалося виконати дію на дошці.", "Board action failed.")


def complete_user_flow_diagnostic(api: Stage1ReleaseAccessibleChessAPI | None = None) -> dict[str, Any]:
    """Exercise the coherent Stage 1 user path without OS/NVDA claims."""
    owned_temp = None
    if api is None:
        owned_temp = tempfile.TemporaryDirectory()
        api = Stage1ReleaseAccessibleChessAPI(keymap_path=Path(owned_temp.name) / "keymap.json")
    ...
    try:
        ...
        checks["e4"] = bool(played.get("ok")) and played.get("historyLength") == 1 and "e4" in str(played.get("fen"))
        checks["e4_board"] = any(cell.get("square") == "e4" and cell.get("occupied") for cell in played.get("board") or [])
        checks["black_to_move"] = " b " in str(played.get("fen")) and bool(played.get("moves"))
