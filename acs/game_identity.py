...
def _move_payload(move: MoveNode) -> dict[str, Any]:
    return {
        "san": move.san,
        # Move numbers are presentation/source syntax.  They do not change the
        # recursive chess record and are intentionally excluded from identity.
        "nags": list(move.nags),
...
def _record_payload(game: PgnGame) -> dict[str, Any]:
    # ``source_index`` and parser warnings describe provenance/import quality,
    # not game identity.  Tag order is irrelevant, while values remain exact.
    return {
        "identity_schema": IDENTITY_SCHEMA_VERSION,
...
def same_game_tree(left: PgnGame, right: PgnGame) -> bool:
    """True when recursive game content matches regardless of PGN formatting."""
...
def same_game_record(left: PgnGame, right: PgnGame) -> bool:
    """True when both recursive content and semantic PGN tags match."""
