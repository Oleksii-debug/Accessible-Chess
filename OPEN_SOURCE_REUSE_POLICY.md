...
Immediate candidates: future Web/PWA may evaluate `chess.js` BSD-2 for legality/FEN/PGN and permissive board widgets such as `gchessboard` MIT or `chessboard-element` MIT, but only if they pass our stricter NVDA semantic/keyboard contract. Do not replace the current WebView2 semantic Stage-1 UI just to adopt a widget. Do not embed Lichess Chessground or Lichess PGN Viewer into a closed-source web client because they are GPL.

Maintain a third-party inventory with upstream, exact version/commit, license, integration mode, local usage, notices/source obligations and security/update owner. Treat external projects as bricks behind ports/adapters, not as the architecture. Never replace working Accessible Chess code wholesale merely because an external library exists; adopt only when migration is smaller and safer, and require regression/differential tests.