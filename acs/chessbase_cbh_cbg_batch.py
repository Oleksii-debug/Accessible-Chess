...
def project_cbh_records_to_cbg_payload_evidence(
    ...
    return _project_cbh_records_with_linker(
        records,
        lambda record: link_cbh_record_to_cbg_payload(record, cbg_data),
    )


def _project_cbh_records_with_linker(
    records: Iterable[ClassicCbhRecord],
    linker: Callable[[ClassicCbhRecord], ClassicCbhCbgPayloadLink],
) -> ClassicCbhCbgBatchProjection:
