...
internal sealed class SentencePack : ISentenceCorpus
{
    ...
    public IReadOnlyList<SentenceRecord> LookupByEntryId(string entryId)
    {
        ...
        return key.Length > 0 && _byEntryId!.TryGetValue(key, out List<SentenceRecord>? values) ? values : Array.Empty<SentenceRecord>();
    }
    ...
    private void BuildIndexes()
    {
        ...
        foreach (SentenceRecord sentence in Sentences)
        {
            ...
            foreach (string lemma in sentence.Lemmas.Select(SentenceTokenizer.NormalizeToken).Where(x => x.Length > 0).Distinct(StringComparer.OrdinalIgnoreCase))
...
internal sealed class SentenceRecord
{
    ...
    public void Validate()
    {
        ...
        if (Lemmas.Count != Tokens.Count)
            throw new InvalidDataException($"Sentence {Id} must have one lemma per normalized token.");
        if (Lemmas.Any(lemma => string.IsNullOrWhiteSpace(lemma)))
            throw new InvalidDataException($"Sentence {Id} contains a blank lemma.");
        foreach (string lemma in Lemmas)
            SentenceTokenizer.ValidateUnicode(lemma, $"Sentence {Id} lemma");

        if (TargetEntryIds.Count == 0)
            throw new InvalidDataException($"Sentence {Id} does not index any target dictionary entries.");
        if (TargetEntryIds.Any(entryId => string.IsNullOrWhiteSpace(entryId) || !string.Equals(entryId, entryId.Trim(), StringComparison.Ordinal)))
            throw new InvalidDataException($"Sentence {Id} contains a blank or non-canonical target entry ID.");
        ...
        if (QualityFlags.Any(flag => string.IsNullOrWhiteSpace(flag)))
            throw new InvalidDataException($"Sentence {Id} contains a blank quality flag.");
    }

    private static bool ContainsLineBreakOrTab(string value) =>
...
internal static partial class SentenceTokenizer
{
    ...
    {
        try { _ = (value ?? string.Empty).Normalize(NormalizationForm.FormKC); }
        catch (ArgumentException ex) { throw new InvalidDataException($"{description} contains malformed Unicode.", ex); }
    }
...
internal static class SentenceAnswerEvaluator
{
    public static SentenceAnswerResult Evaluate(string requiredEnglish, string typedEnglish)
    {
        ...
        {
            return new(false, false, Array.Empty<string>(), Array.Empty<string>(), Array.Empty<string>(),
