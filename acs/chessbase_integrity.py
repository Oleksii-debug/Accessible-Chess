...
from .chessbase_adapter import (
    ChessBaseSourceProbe,
    probe_chessbase_source,
    report_safe_path_name,
)
...
class ChessBaseIntegritySnapshot:
    ...
    def as_report_fields(self) -> dict[str, object]:
        return {
            "primary_path": report_safe_path_name(self.primary_path),
...
def _fingerprint(path: Path, extension: str, role: str) -> SourceFileEvidence:
    safe_name = report_safe_path_name(path)
    try:
        before = path.lstat()
    ...
    if not stat.S_ISREG(before.st_mode):
        raise ChessBaseIntegrityIOError("ChessBase source evidence must be a regular file")
    ...
    if (
        ...
        raise ChessBaseSourceChangedError("ChessBase source changed while integrity evidence was being collected")
...
def capture_integrity_snapshot(path: str | Path) -> ChessBaseIntegritySnapshot:
    ...
    if not probe.recognized:
        raise ValueError(f"Unsupported ChessBase-family source: {report_safe_path_name(probe.path)}")
    ...
    return ChessBaseIntegritySnapshot(primary_path=files[0].path, files=files)
