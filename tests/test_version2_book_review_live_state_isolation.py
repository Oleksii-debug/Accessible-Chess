...
class Version2BookReviewLiveStateIsolationTests(unittest.TestCase):
    ...
    def test_book_review_projection_must_not_replace_live_game_or_history(self) -> None:
        ...
        # The one accessible 64-square board must present the Book review position.
        self.assertEqual(self.api.get_state()["fen"], reviewed_fen)

        # Book reading/review is presentation state. Opening it must not erase or
        # renumber the live game, move list, or ReviewHistory identity underneath.
