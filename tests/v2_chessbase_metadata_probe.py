...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="accessible-chess-metadata-") as temporary:
        ...
        try:
            ...
            # The independent PGN and libcbh may serialize equivalent player
            # name tokens in a different order. Semantic comparison above is
            # token-order-insensitive, while Library player search is a literal
            # normalized substring query. Probe a token from the actual stored
            # canonical record instead of demanding one particular display order.
            stored_first = parse_games(str(rows[0]["pgn_text"]))[0]
            stored_player = stored_first.tags.get("White", "").strip() or stored_first.tags.get("Black", "").strip()
            player_tokens = re.findall(r"[^\W_]+", stored_player, flags=re.UNICODE)
            if not player_tokens:
                raise AssertionError("imported source_index=0 has no searchable player token")
            player_probe = max(player_tokens, key=len)
