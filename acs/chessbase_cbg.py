...
def _parse_cbg_header_bytes(
    raw_header: bytes,
    *,
    offset: int,
    file_size: int,
) -> ClassicCbgGameHeader:
    """Decode one fixed header against a separately established file size."""

    _require_offset(offset)
    if len(raw_header) != _HEADER_SIZE:
        raise CbgDecodeError(
            f"CBG game header at offset {offset} is truncated: "
            f"need {_HEADER_SIZE} bytes, file size is {file_size}"
        )

    raw_size_info = int.from_bytes(raw_header, "big")
...
def _read_cbg_header(
    ...
    *,
    ...
    _require_offset(offset)
    stream.seek(offset)
    raw_header = stream.read(_HEADER_SIZE)
    return _parse_cbg_header_bytes(
        raw_header,
        offset=offset,
        file_size=file_size,
    )
...
def parse_cbg_game_header(data: bytes, *, offset: int) -> ClassicCbgGameHeader:
    ...
    return _parse_cbg_header_bytes(
        data[offset:offset + _HEADER_SIZE],
        offset=offset,
        file_size=len(data),
    )


def _decode_cbg_custom_setup_prefix(
    prefix: bytes,
    *,
    header: ClassicCbgGameHeader,
) -> ClassicCbgSetup:
    ...
    if len(prefix) < _SETUP_END:
        raise CbgDecodeError(
            f"CBG custom setup at offset {offset} is truncated: "
            f"need {_SETUP_END} bytes, got {len(prefix)}"
        )

    metadata = prefix[_SETUP_METADATA_OFFSET]
    castling = prefix[_SETUP_CASTLING_OFFSET]
    next_move_number = prefix[_SETUP_MOVE_NUMBER_OFFSET]
    setup_bytes = bytes(prefix[_SETUP_BITSTREAM_OFFSET:_SETUP_END])
...
def parse_cbg_custom_setup(data: bytes, *, offset: int) -> ClassicCbgSetup:
    ...
    return _decode_cbg_custom_setup_prefix(
        data[offset:offset + _SETUP_END],
        header=header,
    )


def read_cbg_game_header(path: str | Path, *, offset: int) -> ClassicCbgGameHeader:
    """Read only one fixed CBG header without loading the source file."""
...
def read_cbg_custom_setup(path: str | Path, *, offset: int) -> ClassicCbgSetup:
    """Read only the fixed 32-byte custom setup prefix from a CBG source."""

    source = Path(path)
    with source.open("rb") as stream:
        file_size = _stream_size(stream)
        header = _read_cbg_header(stream, offset=offset, file_size=file_size)
        stream.seek(offset)
        prefix = stream.read(_SETUP_END)
        return _decode_cbg_custom_setup_prefix(prefix, header=header)
