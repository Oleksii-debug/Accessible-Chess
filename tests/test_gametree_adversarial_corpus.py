...
class GameTreeAdversarialCorpusTests(unittest.TestCase):
    def test_generated_nested_sibling_corpus_round_trips_with_stable_addresses(self):
        for seed in range(64):
            with self.subTest(seed=seed):
                ...
                self.assertEqual(
                    len(_line_paths(game)),
                    len(set(_line_paths(game))),
                )
                ...
                self.assertEqual(
                    addresses,
                    tuple(iter_move_addresses(reparsed)),
                )
                ...
                for path in _line_paths(game):
                    ...
                    self.assertEqual(
                        context.resume_parent_move_index,
                        step.parent_move_index + 1,
                    )

    def test_generated_edit_compositions_have_total_deterministic_cursor_maps(self):
        for seed in range(24):
            with self.subTest(seed=seed):
                text = (
                    '[Event "Edit corpus"]\n[Result "*"]\n\n'
                    f'Root{seed} '
                    f'(A{seed} A{seed}x (A{seed}n)) '
                    f'(B{seed} B{seed}x) '
                    f'(C{seed} C{seed}x) Tail{seed} *\n'
                )
                source = parse_games(text)[0]
                ...
                self.assertEqual(
                    len({entry.before for entry in result.cursor_remap}),
                    source_cursor_count,
                )
                ...
                self.assertTrue(
                    same_game_record(result.game, parse_games(edited_wire)[0])
                )
                self.assertEqual(serialize_game(source), source_wire)

                repeated_target = variation_edit_target(source, (), 0, target_index)
                repeated = (
                    reorder_variation(
                        source,
                        repeated_target,
                    ...
                    if operation == 0
                    else delete_variation(source, repeated_target)
                    if operation == 1
                    else promote_variation(source, repeated_target)
                )
                self.assertEqual(serialize_game(repeated.game), edited_wire)
                self.assertEqual(repeated.cursor_remap, result.cursor_remap)
        ...
        for seed in range(12):
            with self.subTest(seed=seed):
                ...
                first = reorder_variation(
                    source,
                    variation_edit_target(source, (), 0, 2),
                    0,
                )
                second = promote_variation(
                    first.game,
                    variation_edit_target(first.game, (), 0, 0),
                )
                third = delete_variation(
                    second.game,
                    variation_edit_target(second.game, (), 0, 2),
                )
                ...
                self.assertTrue(
                    same_game_record(third.game, parse_games(final_wire)[0])
                )
                self.assertEqual(
                    [move.san for move in third.game.line.moves],
                    [f'C{seed}', f'Cx{seed}'],
                )
    ...
    def test_malformed_delimiter_corpus_is_bounded_and_blocks_normalization(self):
        ...
        for repeat in range(1, 9):
            for movetext, expected in corpus:
                with self.subTest(repeat=repeat, expected=expected):
                    text = '[Result "*"]\n\n' + (' '.join([movetext] * repeat))
    ...
    def test_small_fixed_resource_envelopes_raise_domain_errors_not_recursion(self):
        ...
        nested = 'A (B (C (D))) *'
        with patch('acs.gametree.MAX_VARIATION_DEPTH', 2):
            with self.assertRaises(GameTreeContractError) as blocked:
                parse_games('[Result "*"]\n\n' + nested)
            self.assertEqual(
                blocked.exception.code,
