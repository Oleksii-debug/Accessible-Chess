...
internal sealed class ProtectedAssessmentRuntime
{
    ...
    private void ValidateExposureBindings()
    {
        foreach (ProtectedAssessmentExposure exposure in _bindingState.Exposures)
        {
            ProtectedAssessmentItemBinding binding = _contract.Resolve(exposure.ItemKey);
            if (!string.Equals(exposure.ExposureGroupId, binding.ExposureGroupId, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Protected assessment exposure group does not match the active form contract.");
        }
    }
    ...
    private void ValidateSessionOrderAgainstContract(AssessmentSessionState session)
    {
        ...
        for (int i = 0; i < session.ItemOrder.Count; i++)
        {
            ...
            bool validReplacement = actual.IsReserve &&
                ...
                !string.Equals(actual.ExposureGroupId, expected.ExposureGroupId, StringComparison.OrdinalIgnoreCase);
            if (!validReplacement)
