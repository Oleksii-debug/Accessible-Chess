...
internal static class SpellingR3IntegrationSelfTest
{
    ...
    {
        // Stage 5 explicitly requires a persisted no-repeat shuffle bag for Recall.
        // Stage 6 does not impose that contract on Spelling. The v1 Spelling
        // contract therefore remains independent random selection from the
        // active scope/deck, with the current implementation avoiding an
        // immediate repeat when more than one eligible word exists. We persist
        // the current card, not an invented remaining-random-sequence state.
