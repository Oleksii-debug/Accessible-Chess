...
class ClassicCbhCbgBatchProjectionTests(unittest.TestCase):
    ...
    def test_file_projection_preserves_both_source_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            projection = read_cbh_cbg_batch_projection(
                cbh_path,
                cbg_path,
    ...
    def test_file_projection_never_reads_past_requested_cbh_window(self):
        first = _cbg_game(b"first")
        second_offset = len(first)
        cbg_bytes = first + _cbg_game(b"second")
        cbh_bytes = _cbh_file(
            [
                _raw_cbh_record(game_offset=0),
                _raw_cbh_record(game_offset=second_offset),
            ]
        )

        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            cbh_path = root / "window.cbh"
            cbg_path = root / "window.cbg"
            cbh_path.write_bytes(cbh_bytes)
            cbg_path.write_bytes(cbg_bytes)
        ...
        self.assertEqual(projection.items[0].record_index, 2)
        self.assertEqual(projection.items[0].link.payload.payload_bytes, b"second")
