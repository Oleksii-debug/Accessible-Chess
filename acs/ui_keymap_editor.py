...
class KeymapEditorModel:
    ...
    def rows(self, *, query: str = "", context: BindingContext | None = None) -> tuple[EditorRow, ...]:
        ...
        for item in projection:
            ...
            if term and term not in haystack:
                continue
            labels = _CONTEXT_LABELS_EN if self.lang == "en" else _CONTEXT_LABELS_UK
            rows.append(EditorRow(str(item["id"]), label, registry_context, labels.get(registry_context, registry_context), current_text, default_text, "shortcut" if item["binding"] is not None else "alias", current_text != default_text))
        return tuple(rows)
    ...
    def reset_context(self, context: BindingContext) -> EditorResult:
        ...
        return EditorResult(True, f"Defaults restored for {label}." if self.lang == "en" else f"Стандартні значення відновлено для: {label}.")
    ...
    def _conflict_message(self, conflicts: Iterable[Conflict]) -> str:
        ...
        if not items:
            return ""
        return ("Conflict: " if self.lang == "en" else "Конфлікт: ") + "; ".join(item.message for item in items)
