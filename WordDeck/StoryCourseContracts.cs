...
internal static class StoryCourseContractValidator
{
    ...
    public static void Validate(StoryCourseManifestContract manifest, DictionaryPackage dictionary)
    {
        ...
        if (manifest.ClaimsCompleteEnglishCourse)
        {
            if (manifest.CurriculumAuthority != StoryCourseCurriculumAuthority.ApprovedCurriculum)
                throw new InvalidDataException("Only approved curriculum may claim Complete English authority.");
            RequireCompleteEnglishEligibleProvenance(manifest.Provenance, manifest.CourseId);
        }
        ...
        StoryCourseCurriculumAuthority curriculumAuthority)
    {
        ...
        foreach (StoryCourseNarrativeContract context in unit.DialogueOrStory)
        {
            context.Validate(unit.UnitId);
            if (claimsCompleteEnglishCourse)
                RequireCompleteEnglishEligibleProvenance(context.Provenance, context.ContentId);
        }
    ...
    private static void RequireCompleteEnglishEligibleProvenance(StoryCourseProvenanceContract provenance, string ownerId)
    {
        if (provenance.Origin is StoryCourseContentOrigin.GeneratedFixture or StoryCourseContentOrigin.LearnerLocal)
            throw new InvalidDataException($"{ownerId} fixture/learner-local provenance cannot participate in a Complete English curriculum claim.");
    }
