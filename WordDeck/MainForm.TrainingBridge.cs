...
internal sealed partial class MainForm
{
    ...
    internal void ShowCurrentWordDeckHelp()
    {
        ...
        string help =
            ...
            "Open Story / Narrative Course from the Tools menu. The built-in offline course is organized from A1 through C1. Each chapter states its content origin, repeats selected target vocabulary, binds targets to stable dictionary IDs and offers bounded course tasks after the story. Previous/Next chapter buttons provide linear navigation, while Next recommended follows CEFR progression and later accepts vocabulary/grammar weakness evidence for mastery-based recycling. Mark complete records Story progress and queues follow-up routing contracts for Recall, Spelling, Sentence and Grammar without changing those modes' statistics by itself.\r\n\r\n" +
            "PERSONAL PROGRESS AND UPDATE SAFETY\r\n" +
            "Personal progress is stored outside the program ZIP under %LOCALAPPDATA%\\WordDeck, so replacing the program ZIP does not intentionally erase progress. File > Export personal progress profile and the Tools profile command export one complete personal profile containing Recall, Spelling, Sentence and Story/Narrative Course learning state; the canonical dictionary, audio, SentencePack and built-in story content are not copied into that profile. Import validates the profile, creates recovery material before replacement and applies it through the unified profile service. Older profiles preserve learning modes that did not yet exist in their schema rather than silently clearing them.\r\n\r\n" +
            "Deck > Hide current word removes a word only from normal Recall study. It does not delete the canonical dictionary, audio or saved deck assignments. Hidden words can be restored individually or all at once. File > Reset Recall learning data resets Recall learning overlays only and creates recovery material first; it is not described as a global Spelling/Sentence/Story reset.\r\n\r\n" +
