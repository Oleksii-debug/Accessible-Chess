...
def _selected_references(
    source: Path,
    indexes: tuple[int, ...],
) -> tuple[dict[int, PgnGame], tuple[int, str, str] | None]:
    """Parse only bounded sample records; Product owns every PGN semantic."""

    targets = set(indexes)
    references: dict[int, PgnGame] = {}
    literal: tuple[int, str, str] | None = None
    with source.open("r", encoding="utf-8", errors="strict", newline="") as stream:
        for ordinal, record in _records(stream, STANDARD_GAMES):
            needs_literal_probe = literal is None and any(
                marker in record for marker in ("%", "_", "\\")
            )
            if ordinal not in targets and not needs_literal_probe:
                continue
            parsed = parse_pgn_text(record, strict=True)
            if len(parsed) != 1:
                raise AssertionError(f"reference record {ordinal} was not one game")
            game = parsed[0]
            if ordinal in targets:
                references[ordinal] = game
            if needs_literal_probe:
                observed = _literal_case((game,))
                if observed is not None:
                    _local_index, field, value = observed
                    literal = (ordinal, field, value)
            if len(references) == len(targets) and literal is not None:
                break
    if set(references) != targets:
        raise AssertionError("bounded reference sample is incomplete")
    return references, literal
...
def _stored_game(database: AcsDatabase, game_id: int) -> PgnGame:
    ...
    games = parse_pgn_text(row["pgn_text"], strict=True)
    if len(games) != 1:
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="accessible-chess-real-v6-") as raw_root:
        ...
        reference_games, literal = _selected_references(standard_pgn, sample_indexes)
        ...
        try:
            ...
            streaming_result = StreamingPgnLibraryImporter(
            ...
            standard_result = streaming_result.library
            if not streaming_result.complete:
                raise AssertionError("streaming importer reported an incomplete source")
            if streaming_result.accepted_games != STANDARD_GAMES:
                raise AssertionError("streaming parser did not accept exactly 10,000 games")
            ...
            for source_index in sample_indexes:
                ...
                reference = reference_games[source_index]
        ...
        summary = {
            "status": "PASS",
            "product_candidate": os.environ.get("V2_STREAMING_HEAD", "UNSET"),
            "schema": 6,
            "standard_corpus": {
                "name": STANDARD.name,
                ...
                "subset_sha256": streaming_result.source.sha256,
                "subset_bytes": streaming_result.source.size,
                "warning_games": standard_result.warning_count,
            },
            "unicode_corpus": {
                "name": UNICODE.name,
            ...
            "library": {
                ...
                "streaming_parse_and_import_seconds": standard_import_seconds,
