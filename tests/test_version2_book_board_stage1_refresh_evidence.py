...
class Version2BookBoardStage1RefreshEvidenceTests(unittest.TestCase):
    """Convergence oracle for Book projection plus V2 DOM routing.

    The canonical BookBoard position is projected by the Product backend.  Once
    the V2 route returns to the original Stage1 board, that existing DOM must be
    refreshed through its canonical ``window.refreshState()`` renderer before
