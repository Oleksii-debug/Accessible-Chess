...
class _BoundedBytesIO(io.BytesIO):
    ...
    def read(self, size=-1):
        if size < 0:
            raise AssertionError("reader attempted an unbounded CBT read")
...
class ClassicCbtTournamentTests(unittest.TestCase):
    ...
    def test_file_reader_reads_only_version_header_and_requested_record(self):
        stream = _BoundedBytesIO(
            _cbt_image(
                version=4,
                tournaments=(("First", "Site"), ("Second", "Place")),
            )
        )
        with mock.patch("acs.chessbase_cbt.Path.open", return_value=stream):
            tournament = read_cbt_tournament("ignored.cbt", tournament_no=1)

        self.assertEqual((tournament.event, tournament.site), ("Second", "Place"))
        self.assertEqual(stream.read_sizes, [0x19, CBT_RECORD_SIZE])
