...
- Two new authoritative source-backed batches are now staged and verified for the next grouped activation: **29 C1 lexical rows from `donor` through `embarrassment`** plus **29 C1 lexical rows from `embassy` through `equality`**. They preserve explicit POS distinctions (`ease` noun/verb and `echo` noun/verb) and deterministic SHA-256 stable IDs.
- Runtime-safe/source-reviewed Oxford 5000 additions are therefore **490 active + 58 newly verified staged = 548 source-checked additions available for the next grouped activation**. The 58 staged rows are deliberately not counted as runtime-active until bootstrap/self-tests and the grouped Windows gate verify the activation checkpoint.
- Oxford official list confirmation for both new staged spans was rechecked on 2026-08-19. Potentially ambiguous lexical rows such as `driving` adjective, `dynamic` noun, `dumb` adjective, `ease` noun/verb and other broad-sense items were handled conservatively against current OALD entries rather than inferred from spelling alone.
...
- **Exact next data action:** continue authoritative extraction after `equality` in another large verified batch, then group multiple staged source batches into one runtime activation/Windows gate rather than spending CI minutes on every 29-row slice.
...
- Oxford 5000 MP3 coverage remains **0/490 runtime additions**; all **58 newly staged rows** also still need generation. This is a release blocker for the emergency final milestone.
...
- Full official Oxford 5000 row-level extraction remains incomplete: 490 runtime-active additions plus 58 newly verified staged additions.
...
- The 58 new rows are intentionally staged without an expensive Windows gate; they will be activated in a larger coherent checkpoint to conserve CI minutes.
