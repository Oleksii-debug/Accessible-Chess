...
class ContinuousAnalysisTests(unittest.TestCase):
    def test_start_defaults_to_multipv5_and_publishes_current_result(self):
        ...
        try:
            ...
            self.assertEqual(state.multipv, 5)
            self.assertEqual(state.depth, 16)
            self.assertEqual(len(results[0].lines), 5)
    ...
    def test_position_change_suppresses_stale_result_and_analyzes_latest(self):
        ...
        engine = RecordingEngine(gate)
        results = []
        service = ContinuousAnalysisService(AnalysisService(lambda: engine), results.append)
        try:
            service.start('fen-old', multipv=2, depth=10)
            self.assertTrue(engine.first_started.wait(timeout=1))
            service.update_position('fen-middle')
            service.update_position('fen-new')
            gate.set()
            self.assertTrue(wait_until(lambda: len(results) == 1))
            self.assertEqual(results[0].fen, 'fen-new')
            self.assertEqual([call[0] for call in engine.calls], ['fen-old', 'fen-new'])
            self.assertEqual(service.state().fen, 'fen-new')
        finally:
            service.close()
    ...
    def test_configure_reanalyzes_current_position_with_new_limits(self):
        ...
        try:
            ...
            self.assertTrue(wait_until(lambda: len(results) == 1))
            service.configure(multipv=5, depth=22)
            self.assertTrue(wait_until(lambda: len(results) == 2))
            self.assertEqual(engine.calls[-1], ('fen-a', 5, 22))
            self.assertEqual(len(results[-1].lines), 5)
            self.assertEqual(results[-1].lines[0].depth, 22)
        finally:
            service.close()
    ...
    def test_stop_invalidates_in_flight_work_and_restart_is_clean(self):
        ...
        try:
            ...
            service.start('fen-restart', multipv=3, depth=12)
            self.assertTrue(wait_until(lambda: len(results) == 1))
            self.assertEqual(results[0].fen, 'fen-restart')
            self.assertEqual(len(results[0].lines), 3)
        finally:
            service.close()
    ...
    def test_closed_service_rejects_new_work_and_closes_engine(self):
        ...
        with self.assertRaises(RuntimeError):
            service.start('fen-b')
        with self.assertRaises(RuntimeError):
            service.update_position('fen-b')
        with self.assertRaises(RuntimeError):
            service.configure(depth=20)
    ...
    def test_callback_failure_does_not_kill_worker(self):
        ...
        try:
            service.start('fen-a')
            self.assertTrue(wait_until(lambda: seen == ['fen-a']))
            service.update_position('fen-b')
            self.assertTrue(wait_until(lambda: seen == ['fen-a', 'fen-b']))
            self.assertEqual(service.state().last_result.fen, 'fen-b')
        finally:
            service.close()
