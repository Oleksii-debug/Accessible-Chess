        ...
        DictionaryPackage dictionary)
    {
        ...
        if (starts.Count == 0)
        {
            throw new InvalidDataException(
                $"Sentence {card.SentenceId} indexes target {focusId}, but exact physical lexical form '{lexicalKey}' is not present in the canonical sentence token stream. Target-form spelling fails closed rather than guessing morphology.");
        }
