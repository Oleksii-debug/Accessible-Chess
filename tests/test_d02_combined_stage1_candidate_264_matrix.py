...
# Evidence-only matrix. This file deliberately mutates no Product code.
# 264 independently discoverable unittest methods exercise D02-owned canonical
# state/history/FEN/atomicity contracts on the exact combined Stage1 repair.
...
class D02CombinedStage1Candidate264Matrix(unittest.TestCase):
    pass


def _make_square_roundtrip_case(index: int):
    def test(self: D02CombinedStage1Candidate264Matrix) -> None:
...
for _index in range(64):
    setattr(
        D02CombinedStage1Candidate264Matrix,
...
def _assert_matrix_shape() -> None:
    discovered = [name for name in dir(D02CombinedStage1Candidate264Matrix) if name.startswith("test_")]
    if len(discovered) != MATRIX_CASE_COUNT:
        raise RuntimeError(f"D02 candidate matrix shape drift: expected {MATRIX_CASE_COUNT}, got {len(discovered)}")
