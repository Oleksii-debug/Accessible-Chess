...
def _batch_error_text(exc: Exception, source: Path) -> str:
    ...
    errno when available, and report-safe filenames. Adapter ``ValueError`` and
    ``RuntimeError`` text is likewise not trusted as user-safe, so batch reports
    a stable source-scoped message while strict ``inspect`` still exposes the
    original exception to internal callers.
    """
