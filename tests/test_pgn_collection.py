...
class PgnCollectionTests(unittest.TestCase):
    ...
    def test_skip_all_collapses_duplicates_inside_one_incoming_batch(self):
        original = PgnWorkspace()
        merged, report = merge_text(
            original,
            GAME_A + "\n" + GAME_A,
            duplicate_policy=DuplicatePolicy.SKIP_ALL,
        )
    ...
    def test_large_five_thousand_game_merge_dedup_and_page_cycle(self):
        ...
        text = "\n".join(records)
        empty = PgnWorkspace()
        merged, report = merge_text(empty, text, duplicate_policy=DuplicatePolicy.SKIP_ALL)
