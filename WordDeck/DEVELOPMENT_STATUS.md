...
### Oxford lexical data
- Runtime production bridge is now **3,624 rows = 3,308 unchanged Oxford 3000 baseline rows + 316 verified canonical B2/C1 Oxford 5000 rows**. Existing Oxford 3000 IDs/progress remain unchanged and the durable dictionary ID remains `oxford-3000-en-uk` for lossless migration.
- Three consecutive post-bridge C1 slices are runtime-eligible: **43/43** rows `boast` verb C1 through `chamber` noun C1, **29/29** rows `chaos` noun C1 through `colonial` adjective C1, and **29/29** rows `columnist` noun C1 through `compute` verb C1. All three batches passed source/POS/CEFR review and Ukrainian translation QA before activation.
- The latest 29-row batch was checked directly against the official Oxford 3000/5000 word-list page on 2026-08-18. It preserves distinct POS rows such as `combat` noun/verb and `compromise` noun/verb rather than collapsing lexical identities.
...
- Current source-backed and runtime-safe verified frontier reaches **`compute` verb C1**. B2-exclusive membership beyond the existing audited bridge still requires Oxford 5000-specific membership/source metadata rather than inference from CEFR alone.
- Exact next data action: continue the next large C1 source-backed batch after `compute`, while keeping the separate B2-exclusive extraction path tied to official Oxford 5000 membership metadata.
...
- Canonical Oxford 5000 additions now have **316 runtime-stable lexical IDs**.
- **Oxford 5000 addition MP3 coverage remains 0/316 generated files at this checkpoint.** Exact next audio action: generate one meaningful British batch for activated additions with stable-ID manifest and SHA-256 integrity, then append later verified rows only after lexical QA threshold. Do not wholesale-regenerate Oxford 3000.
...
- Full-data blocker: complete official Oxford 5000 row-level extraction is still incomplete. Runtime-safe verified additions increased from 287 at run start to **316**.
