...
class Stage1CompleteUserFlowTests(unittest.TestCase):
    ...
    def test_fen_and_position_errors_never_expose_python_exception_text(self):
        api = self.make_api()
        for result in (api.set_fen("bad"), api.set_position_text("bad", "w"), api.activate_square("z9")):
    ...
    def test_release_launcher_uses_hardened_api(self):
        import run_accessible_chess
        # Import with unittest argv follows the non-diagnostic branch only when
        # executed as an app, so the contract is source-level here.
        source = Path(run_accessible_chess.__file__).read_text(encoding="utf-8")
