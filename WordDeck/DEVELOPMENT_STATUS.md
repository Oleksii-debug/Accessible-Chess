...
### Oxford lexical data
- Runtime production bridge is now **3,682 rows = 3,308 unchanged Oxford 3000 baseline rows + 374 verified canonical B2/C1 Oxford 5000 rows**. Existing Oxford 3000 IDs/progress remain unchanged and the durable dictionary ID remains `oxford-3000-en-uk` for lossless migration.
- Five consecutive post-bridge C1 slices are runtime-eligible: **43/43** rows `boast` verb C1 through `chamber` noun C1, **29/29** rows `chaos` noun C1 through `colonial` adjective C1, **29/29** rows `columnist` noun C1 through `compute` verb C1, **29/29** rows `conceal` verb C1 through `constitution` noun C1, and **29/29** rows `constitutional` adjective C1 through `correlation` noun C1.
- The newest 29-row slice was checked against the official Oxford 3000/5000 list on 2026-08-19 for exact headword/POS/CEFR membership. Ambiguous/polysemous entries including `content` adjective, `contrary` adjective/noun, `contend`, `contention`, `conversion` and `conviction` received targeted Oxford Advanced Learner's Dictionary definition checks before Ukrainian gloss activation. Distinct POS rows remain distinct stable lexical identities.
...
- Current source-backed and runtime-safe verified C1 frontier reaches **`correlation` noun C1**. B2-exclusive membership beyond the existing audited bridge still requires Oxford 5000-specific membership/source metadata rather than inference from CEFR alone.
- Exact next data action: continue the next large C1 source-backed batch after `correlation`, while keeping the separate B2-exclusive extraction path tied to official Oxford 5000 membership metadata.
...
- Canonical Oxford 5000 additions now have **374 runtime-stable lexical IDs**.
- **Oxford 5000 addition MP3 coverage remains 0/374 generated files at this checkpoint.** Exact next audio action: generate one meaningful British batch for activated additions with stable-ID manifest and SHA-256 integrity, then append later verified rows only after lexical QA threshold. Do not wholesale-regenerate Oxford 3000.
...
- Full-data blocker: complete official Oxford 5000 row-level extraction is still incomplete. Runtime-safe verified additions increased from 345 to **374** in this run.
...
- Verification discipline: the newest 374-row data checkpoint is committed, but this status does not call it a user-testable beta until a grouped Windows build, self-tests, self-contained publish and published-EXE self-test are confirmed green.
