...
internal static class ReleaseRegressionSelfTest
{
    ...
    private static void TestCompatibleOlderSameCorpusProfileMigrates()
    {
        ...
        try
        {
            ...
            Require(current.SchemaVersion == AppStateStore.CurrentSchemaVersion,
                "Compatible older same-corpus profile did not migrate to the current state schema.");
            Require(current.HiddenEntryIds.Contains("known-id"),
                "Compatible older same-corpus profile lost progress during migration.");
            Require(File.Exists(result.BackupPath),
                "Compatible profile import did not create a pre-import recovery profile after compatibility validation.");
        }
        finally { DeleteTree(root); }
    }
    ...
    {
        string root = Path.Combine(Path.GetTempPath(), $"WordDeck-r2-release-{suffix}-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        return root;
    }
