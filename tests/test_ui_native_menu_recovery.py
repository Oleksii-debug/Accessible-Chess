...
def test_native_recovery_resets_core_then_clears_only_legacy_keymap_cache() -> None:
    ...
    window = FakeWindow()
    menus = make_keymap_menu(fake_webview(), api, {"window": window})
    recovery = next(item for item in _settings_actions(menus) if item.title == "Reset all keyboard commands")
    recovery.callback()
...
def test_host_resolution_prefers_the_form_that_actually_owns_webview2() -> None:
    ...
    class WebViewControl:
        ...
        def FindForm(self):
            return real_form
    detached_wrapper = SimpleNamespace(Controls=[], MainMenuStrip=None, TopLevel=True, webview=WebViewControl())
