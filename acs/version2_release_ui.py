...
def _close_release_resources(
    api: Version2ReleaseAccessibleChessAPI,
    application: Any,
    runtime: Any | None,
) -> None:
    """Close all owned V2 release resources, even after partial startup.

    Resource preflight, pywebview import/window creation, native-menu ownership and
    JavaScript bootstrap all happen after the application/engine composition root
    already exists.  Any exception in that interval must therefore take the same
    shutdown path as a normal WebView exit so no database/analysis/Stockfish
    resource survives a failed launch.
    """
...
def run_version2_release_window(
    ...
    *,
    ...
    try:
        ...
        _close_release_resources(api, application, runtime)
