...
class CbvExternalExtractorTests(unittest.TestCase):
    ...
    def _successful_runner(self, _executable, arguments, _config, *, cwd, monitor_directory=None):
        if arguments[0] == "list":
    ...
    def test_validated_archive_extracts_one_classic_cbh_family(self) -> None:
        with mock.patch("acs.cbv_extractor._run_uncbv", side_effect=self._successful_runner):
            result = extract_cbv_external(self.source, self.output, self.config)

        self.assertEqual(result.source.sha256, sha256(self.source.read_bytes()).hexdigest())
        self.assertEqual(result.primary_path, self.output / "nested" / "Training.cbh")
    ...
    def test_traversal_absolute_drive_and_case_collisions_fail_before_extraction(self) -> None:
        unsafe_lists = (
            b"../escape.cbh\n",
            b"/absolute.cbh\n",
