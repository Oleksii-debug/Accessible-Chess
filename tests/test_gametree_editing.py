...
class GameTreeEditingTests(unittest.TestCase):
    ...
    def test_reorder_is_copy_on_write_and_remaps_every_source_cursor(self):
        ...
        old_nested = GameTreeCursor((VariationStep(1, 0), VariationStep(1, 0)), 1)
        self.assertEqual(
            result.remap_cursor(old_nested),
            GameTreeCursor((VariationStep(1, 1), VariationStep(1, 0)), 1),
        )
    ...
    def test_delete_maps_only_removed_subtree_context_to_none(self):
        ...
        self.assertIsNone(result.remap_cursor(GameTreeCursor((VariationStep(1, 1),), 1)))
        self.assertEqual(
            result.remap_cursor(GameTreeCursor((VariationStep(1, 2),), 1)),
            GameTreeCursor((VariationStep(1, 1),), 1),
        )
        self.assertEqual(result.remap_cursor(GameTreeCursor((), 3)), GameTreeCursor((), 3))
        self.assert_source_unchanged()
    ...
    def test_promote_swaps_mainline_and_preserves_siblings_nested_lines_and_context(self):
        ...
        self.assertEqual([move.san for move in edited.line.moves], ['e4', 'c5', 'Nf3', 'Nc6'])
        ...
        self.assertEqual(
            [move.san for move in resolve_line(edited, (VariationStep(2, 0),)).moves],
            ['Nc3', 'd6'],
        )
        self.assertEqual(
        ...
        self.assertEqual(
            result.remap_cursor(GameTreeCursor((VariationStep(1, 0), VariationStep(1, 0)), 1)),
        ...
        self.assert_source_unchanged()
        self.assertTrue(same_game_record(edited, parse_games(serialize_game(edited))[0]))
    ...
    def test_reorder_permutations_keep_cursor_remap_total_and_deterministic(self):
        for source_index in range(3):
            for destination_index in range(3):
                first = reorder_variation(self.game, self.target(source_index), destination_index)
                second = reorder_variation(self.game, self.target(source_index), destination_index)
                self.assertEqual(serialize_game(first.game), serialize_game(second.game))
                self.assertEqual(first.cursor_remap, second.cursor_remap)
                self.assertTrue(all(entry.after is not None for entry in first.cursor_remap))
