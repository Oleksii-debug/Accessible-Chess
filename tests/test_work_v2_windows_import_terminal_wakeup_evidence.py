...
class Version2WindowsImportTerminalWakeupEvidenceTests(unittest.TestCase):
    """RED-first exact-host oracle for a lost final import UI wakeup.

    Existing pump coverage proves retry after a *later* worker event or after an
    explicit caller invokes request_pending_wakeup().  A terminal event has no
    later worker event.  One transient BeginInvoke scheduling failure must not
    leave the completed import stranded in the mailbox until another user action.
    """
    ...
    def test_terminal_event_self_recovers_after_one_transient_begininvoke_failure(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            # Drain the first worker wakeup so terminal completion must schedule
            # a fresh wakeup rather than sharing the already pending one.
            ...
            # Fail exactly the new BeginInvoke requested by the final worker event.
            ...
            # press Import/Cancel merely to make completion visible to NVDA/UI.
