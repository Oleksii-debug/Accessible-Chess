...
- `interaction-message-v1.schema.json` is a discriminated union. `family` and
  `version` are mandatory; additional fields are rejected. Position-editor
  operations are bounded and operation-specific field combinations are
  validated before mutation authority can be granted.
...
- `distinct-square-pair-v1.schema.json` is the shared arrow invariant used by
  both message and presentation schemas. It rejects all 64 equal start/end
  pairs while allowing distinct canonical squares.
...
- Positive and negative conformance examples live in
  ...
  same-square arrows, unknown position-editor operations, and invalid
  operation-specific field combinations.
