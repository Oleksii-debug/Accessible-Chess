...
internal static class ContextSentenceTargetSpellingFactory
{
        ...
        DictionaryPackage dictionary)
    {
        ...
        // A surface-form sentence can prove that the written form is present, but it
        // cannot choose which Oxford noun/verb/sense ID owns a homographic occurrence.
        // Sentence Spelling records target-specific learning evidence, so it must fail
        // closed until the sentence source carries explicit POS/sense disambiguation.
        if (lexicon.IsAmbiguousStableIdentity(focusId))
        {
            string lexicalKey = lexicon.LexicalKeyFor(focusId);
            string alternatives = string.Join("|", lexicon.StableIdsForLexicalKey(lexicalKey));
            throw new InvalidDataException(
                $"Sentence target spelling cannot use unresolved stable ID {focusId}. " +
                $"Written form '{lexicalKey}' maps to multiple dictionary entries [{alternatives}]; explicit POS/sense evidence is required before target-specific progress may be recorded.");
        }
        ...
        string targetLexicalKey = lexicon.LexicalKeyFor(focusId);
        string[] physicalTokens = SentenceTokenizer.Tokenize(targetLexicalKey).ToArray();
        ...
        {
            throw new InvalidDataException(
                $"Sentence {sentence.Id} indexes target {focusId}, but its exact physical lexical form '{targetLexicalKey}' is not present in the canonical token stream. Target-form spelling fails closed rather than guessing morphology.");
        }
